...
int FST_TRACE_n = -1;
char rbuf[205], sbuf[205], lbuf[1024]; // ïå÷à
MfstState::MfstState()
{
	lenta_position = 0;
	nrule = -1;
...
bool	Mfst::start(Log::LOG& log)
{
	...
	case LENTA_END:			MFST_TRACE4("------>LENTA_END")
		...
		*log.stream << std::setw(4) << std::left << 0 << ": âñåãî ñòðîê " << lenta_size << ", ñèíòàêñè÷åñêèé àíàëèç âûïîëíåí áåç îøèáîê" << std::endl;
		rc = true;
		break;
...
char*	Mfst::getDiagnosis(short n, char* buf)
{
	...
	if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
	{
		...
		sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: ñòðîêà %d, %s", errid, lex.table[lpos].sn, err.message);
		rc = buf;
	};
	return rc;
}
...
{
	*log.stream << "Ñèíòàêñè÷åñêèé àíàëèç - Âûïîëíåí" << std::endl;
	MfstState state;
	GRB::Rule rule;
